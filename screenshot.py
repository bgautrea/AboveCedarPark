...
abovecedarpark_image_width = int(parser.get('abovecedarpark', 'image_width'))
abovecedarpark_image_height = int(parser.get('abovecedarpark', 'image_height'))
...
g_request_timeout = float(parser.get('abovecedarpark', 'request_timeout'))
...
class Dump1090Display(AircraftDisplay):
    def loadmap(self):
        ...
        browser.set_window_size(abovecedarpark_image_width, abovecedarpark_image_height)
        ...
        try:
            element = wait.until(EC.element_to_be_clickable((By.ID,'skyaware_version')))
