...
abovecedarpark_distance_alarm = float(parser.get('abovecedarpark', 'distance_alarm'))	# The alarm distance in miles.
abovecedarpark_elevation_alarm = float(parser.get('abovecedarpark', 'elevation_alarm'))	# The angle in degrees that indicates if the airplane is overhead or not.
abovecedarpark_wait_x_updates = int(parser.get('abovecedarpark', 'wait_x_updates'))	# Number of updates to wait after the airplane has left the alarm zone before tweeting.
abovecedarpark_sleep_time = float(parser.get('abovecedarpark', 'sleep_time'))		# Time between each loop.
...
def Tweet(a, havescreenshot):
	...
	templateArgs['alt_ft'] = a.alt_geom
	templateArgs['alt_m'] = "%.1f" % geomath.ft2m(a.alt_geom)
	templateArgs['el'] = "%.1f" % a.el
	templateArgs['az'] = "%.1f" % a.az
	templateArgs['heading'] = geomath.HeadingStr(a.track)
	templateArgs['speed_mph'] = "%.1f" % a.gs
	templateArgs['speed_kmph'] = "%.1f" % geomath.mi2km(a.gs)
	templateArgs['speed_kts'] = "%.1f" % geomath.mi2nm(a.gs)
	templateArgs['time'] = a.time.strftime('%H:%M:%S')
	templateArgs['squawk'] = a.squawk
	templateArgs['vert_rate_ftpm'] = a.baro_rate
	templateArgs['vert_rate_mpm'] = "%.1f" % geomath.ft2m(a.baro_rate)
	...
	if a.alt_geom >= 1000 and a.alt_geom < 2500 and (templateArgs['heading'] == "S" or templateArgs['heading'] == "SW"):
		hashtags.append(" #ProbablyLanding")
	if a.alt_geom > 20000 and a.alt_geom < 35000:
		hashtags.append(" #UpInTheClouds")
	if a.alt_geom >= 35000:
		hashtags.append(" #WayTheHeckUpThere")
	if a.gs > 300 and a.gs < 500:
		hashtags.append(" #MovingQuickly")
	if a.gs >= 500 and a.gs < 770:
...
if __name__ == "__main__":
	...
	alarms = dict() # dictonary of all aircraft that have triggered the alarm
			...
			# the counter is incremented until we hit [abovecedarpark_wait_x_updates]
	...
	while True:
		...
		sleep(abovecedarpark_sleep_time)
		fd.refresh()
		if fd.time == lastTime:
			continue
		lastTime = fd.time
		...
		for a in fd.aircraft:
			...
			if a.distance < abovecedarpark_distance_alarm or a.el > abovecedarpark_elevation_alarm:
				# add it to the current dictionary
				current[a.hex] = a 
				print("{}: {}mi, {}az, {}el, {}alt, {}dB, {}seen".format(
					a.ident_desc(), "%.1f" % a.distance, "%.1f" % a.az, "%.1f" % a.el,
					a.alt_geom, "%0.1f" % a.rssi, "%.1f" % (a.seen or 0)))
				if a.hex in alarms:
		...
		for h, a in alarms.items():
			...
			if not found:
				if a[1] < abovecedarpark_wait_x_updates:
					alarms[h] = (a[0], a[1]+1)
				else:
